...
if __name__ == '__main__':
    ...
    pickle.dump([features, df_labels], open(data_path + 'features_labels_dist_times_{}.p'.format(N), 'wb'))
    
    with open(data_path + 'features_labels_dist_times_{}.p'.format(N), 'rb') as f:
        features, df_labels = pickle.load(f)
        X = np.array([df_i.values for df_i in features])
        y = df_labels['In-hospital_death'].values.copy()
        y[y == -1] = 0
    ...
    np.savez(open(data_path + 'data_dist.npz', 'wb'), X=X_dist, y=y)
    np.savez(open(data_path + 'data_times_negatives.npz', 'wb'), X=X_times, y=y)
