...
def generate_features(df, duration=48.0, timestep=1.0):
    ...
    for t in np.arange(0.0, duration, timestep):
        ...
        for var in static_variables:
            feature_dict['zinvariant_' + var] = df_t[df_t['Variable'] == var]['Value'].iloc(0)
            feature_dict['zmissinv_' + var] = int(np.isnan(feature_dict['zinvariant_' + var][0]))
            #print(var,':',feature_dict['zinvariant_' + var])
...
if __name__ == '__main__':
    ...
    features = Parallel(n_jobs=16)(delayed(generate_features)(df) for _, df in tqdm(raw_data.items(), desc='Generating feature vectors'))
    pickle.dump([features, df_labels], open(data_path + 'features_labels_invariant_{}.p'.format(N), 'wb'))
    
    with open(data_path + 'features_labels_invariant_{}.p'.format(N), 'rb') as f:
        features, df_labels = pickle.load(f)
        X = np.array([df_i.values for df_i in features])
        y = df_labels['In-hospital_death'].values.copy()
        y[y == -1] = 0
    ...
    np.savez(open(data_path + 'data_nmiss_invariant.npz', 'wb'), X=X_nmiss, y=y)
    np.savez(open(data_path + 'data_miss_invariant.npz', 'wb'), X=X_miss, y=y)
